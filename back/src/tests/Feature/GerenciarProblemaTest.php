...
class GerenciarProblemaTest extends TestCase
{
    ...
    /**
     * Caso 2.7: Editar Tempo Limite do Problema
     */
    ...
    /**
     * Caso 2.8: Editar Enunciado do Problema
     */
    public function test_professor_pode_editar_enunciado_problema(): void
    {
        $this->actingAsProfessor();
        ...
        $updatePayload = array_merge($payload, ['enunciado' => 'Problema de Soma Simples (A+B) - Atualizado']);
        $response = $this->putJson(route('problemas.update', $problema->id), $updatePayload);

        $response->assertStatus(200);
        $this->assertDatabaseHas('problema', ['id' => $problema->id, 'enunciado' => 'Problema de Soma Simples (A+B) - Atualizado']);

        $show = $this->getJson(route('problemas.show', $problema->id));
        $show->assertStatus(200)->assertJsonPath('enunciado', 'Problema de Soma Simples (A+B) - Atualizado');
    }


    /**
     * Caso 2.9: Editar Memória Limite
     */
    public function test_professor_pode_editar_memoria_limite_problema(): void
    {
        $this->actingAsProfessor();
        ...
        $updatePayload = array_merge($payload, ['memoria_limite' => 256]);
        $response = $this->putJson(route('problemas.update', $problema->id), $updatePayload);

        $response->assertStatus(200);
        $this->assertDatabaseHas('problema', ['id' => $problema->id, 'memoria_limite' => 256]);
    }


    /**
     * Caso 2.10: Editar Casos de Teste (Entrada/Saída)
     */
    public function test_professor_pode_editar_casos_de_teste_entrada_saida(): void
    {
        $this->actingAsProfessor();
        ...
        // Atualiza o caso de teste: alteração da saída de '4' para '4.0'
        $updatePayload = $payload;
        $updatePayload['casos_teste'] = [ [ 'entrada' => '2 2', 'saida' => '4.0', 'privado' => true ] ];

        $response = $this->putJson(route('problemas.update', $problema->id), $updatePayload);
        $response->assertStatus(200);

        $this->assertDatabaseHas('caso_teste', ['entrada' => '2 2', 'saida' => '4.0']);
    }


    /**
     * Caso 2.11: Editar Visibilidade do Caso de Teste (Público/Privado)
     */
    public function test_professor_pode_editar_visibilidade_caso_de_teste(): void
    {
        $this->actingAsProfessor();
        ...
        $updatePayload = $payload;
        $updatePayload['casos_teste'] = [ [ 'entrada' => '2 2', 'saida' => '4', 'privado' => false ] ];

        $response = $this->putJson(route('problemas.update', $problema->id), $updatePayload);
        $response->assertStatus(200);

        $this->assertDatabaseHas('caso_teste', ['entrada' => '2 2', 'privado' => false]);
    }

    /**
     * Caso 2.12: Excluir Problema
     */
    ...
    /**
     * Caso 2.14: Visualizar detalhes de um problema (Caminho-Feliz)
     */
    ...
    /**
     * Caso 2.15: Visualizar problema inexistente (Falha)
     */
    ...
    /**
     * Caso 2.16: Ação não permitida ao professor (acesso admin)
     */
    public function test_professor_nao_pode_acessar_rotas_admin(): void
    {
        $this->actingAsProfessor();

        $response = $this->getJson('/teachers');
        $this->assertTrue(in_array($response->status(), [403,401,302,404]));
    }

    /**
     * Caso 2.17: Retornar à tela anterior (simulado)
     */
    ...
    /**
     * Caso 2.18: Visualizar lista de problemas
     */
    ...
    /**
     * Caso 2.19: Visualizar lista de problemas sem problemas cadastrados
     */
